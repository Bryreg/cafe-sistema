 ...
 * ABIERTA, que el usuario marca arrastrando una línea sobre un dibujo de bolsa/botella.
 * El total = selladas + nivel (ej: 3 selladas + ½ abierta = 3,5). Controlado por el padre.
 */
interface Props {
  envase: 'bolsa' | 'botella'
  unidad?: string            // etiqueta: 'bolsa' | 'botella' | 'unidad'
  selladas: number
  nivel: number              // 0..1
  onChange: (selladas: number, nivel: number) => void
  color?: string             // acento del fill (opcional)
}
...
export default function NivelEnvase({ envase, unidad, selladas, nivel, onChange, color }: Props) {
  const clipId = useId().replace(/:/g, '')
  const svgRef = useRef<SVGSVGElement>(null)
  const drag = useRef(false)
  const g = GEO[envase]
  const top = g.top
  const y = BOT - nivel * (BOT - top)
  const uWord = unidad ?? envase
  const acc = color ?? g.line
    ...
    <div className="flex items-center gap-3">
      ...
      >
        ...
        <rect x={envase === 'bolsa' ? 26 : 34} y={y} width={envase === 'bolsa' ? 68 : 52} height={BOT - y}
          clipPath={`url(#${clipId})`} fill={g.fill} />
        <line x1={envase === 'bolsa' ? 20 : 28} y1={y} x2={envase === 'bolsa' ? 100 : 92} y2={y}
          stroke={acc} strokeWidth="2.5" strokeDasharray="4 3" />
        <circle cx={envase === 'bolsa' ? 94 : 86} cy={y} r="5" fill={acc} />
      </svg>

      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2 mb-1.5">
          <span className="text-[11px] text-gray-400">Selladas</span>
          <button type="button" onClick={() => selladas > 0 && onChange(selladas - 1, nivel)}
          ...
          <button type="button" onClick={() => onChange(selladas + 1, nivel)}
            className="w-7 h-7 rounded-lg border border-gray-200 text-gray-500 leading-none">+</button>
        </div>
        <p className="text-[11px] text-gray-400">Abierta: <span className="font-semibold" style={{ color: acc }}>{nivelLabel(nivel)}</span></p>
