...
def _base_desde_ultimo_cierre(ultimo, consigs_deducidas: float) -> float:
    ...
    LO QUE DEVUELVE ES PLATA PROPIA: exactamente lo que hay por consignar de ese
    turno. Esa es la invariante que sostiene toda la fórmula del consignable, y
    por eso la reserva de la caja fuerte se declara aparte (`CajaTurno.caja_fuerte`)
    y NO entra acá: si entrara, `base_sistema` dejaría de ser lo que hay que
    bancar y el consignable empezaría a reclamar la reserva de la sede."""
    if ultimo is None:
        return 0.0
...
def registrar_cuadre_inicial(db: Session, turno_id: int, usuario_id: int,
    ...
    if saldos_incluidos is not None:
        esperado, seleccion_detalle = _resolver_saldos_incluidos(
...
def cerrar_caja(db: Session, turno_id: int, efectivo_final_real: float,
    ...
    # La reserva de la caja fuerte NO entra: se declara al abrir (`CajaTurno.caja_fuerte`)
    # y se queda guardada, no pasa por la registradora.
...
def cerrar_turno_administrativo(db: Session, turno_id: int, usuario_id: int,
    ...
    # Misma fórmula del cuadre, menos lo ya consignado.
