...
export default function BannerLibro({
  libro, serie, anio, mes, hoy, viendoElMesDeHoy, cargando, errorLibro,
  cuentas, vencimientos, onIrAlMes, onIrAHoy, onVerMes, onCambiarAnio,
  onGuardado, onBorrado, onIrAlAncla, onPagar, itemPagando, renderPago,
}: {
  libro: LibroMes | null
  serie: SerieAnual | null
  anio: number
  mes: number
  hoy: string
  ...
  errorLibro: string
  cuentas: CuentaBanco[]
  /** La agenda indexada por día: los vencimientos NO están en el libro. */
  vencimientos: Map<string, AgendaItem[]>
  ...
  return (
    ...
    >
      ...
      {/* La verdad PARCIAL: desde cuándo hay saldo. Solo cuando no hay ningún día
          con saldo esto dice «este mes no tiene saldos». */}
      {avisoDeCadena && (
        <button onClick={onIrAlAncla}
          ...
          <span className="min-w-0 flex-1">
            <span className="block text-xs font-bold text-gold-700">{avisoDeCadena.titulo}</span>
      ...
      {(error || errorLibro) && (
        <div className="px-3 py-2"><ErrorCampo msg={error || errorLibro} /></div>
      )}
      ...
      {!cargando && libro && libro.dias.map(d => (
        <FilaDia key={d.fecha} dia={d} hoy={hoy} columnasDeSaldo={hayColumnasDeSaldo}
          ...
          vencimientos={vencimientos.get(d.fecha) ?? []}
      ...
      {/* ── LOS DOCE MESES ──────────────────────────────────────────────────
          ...
          días es la del mes de arriba. */}
      {serie && (
        <ComoSeCalcula titulo={`Ver el año ${serie.anio} entero`}>
          <div className="flex items-center gap-1 -mt-1">
            ...
            <p className="flex-1 text-center text-xs font-bold text-warm-700">
              Cuánto entró, cuánto salió y con cuánto cerró cada mes de {serie.anio}
            </p>
          ...
          <div className="rounded-xl border border-warm-200 overflow-hidden bg-white">
            ...
            {serie.meses.map(m => {
              ...
              // plata movida se apaga; uno con plata se lee aunque el cierre no
              // exista.
            ...
            {mesesSinCierre > 0 && (<>
              ...
              {libro?.ancla.fecha
                ? `: el cierre se mide el último día del mes, y ${mesesSinCierre === 1
                  ...
                  + `${fechaCorta(libro.ancla.fecha)}, que es la fecha del saldo del extracto.`
 ...
 */
function FilaDia({
  dia, hoy, columnasDeSaldo, abierto, vencimientos, itemPagando, renderPago,
